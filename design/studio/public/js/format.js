...
export function describeEditError(err) {
  if (err) {
    ...
    if (err.code === 'edit_timeout') {
      const budget = typeof err.budgetMs === 'number' ? ` after ${formatElapsed(err.budgetMs)}` : '';
      return (
        `The edit was still running${budget}, so it was cancelled.${LEFT_UNCHANGED} ` +
        'Try a smaller, more specific instruction.'
      );
    }
    if (err.code === 'validation_failed') {
      if (err.reason === 'shrunk') {
        return (
          `The agent returned a page less than half its original size, so the edit ` +
          `was rejected.${LEFT_UNCHANGED} Try a smaller, more specific instruction.`
        );
      }
