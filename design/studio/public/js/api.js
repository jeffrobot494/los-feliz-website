...
export class ApiError extends Error {
  constructor(status, code, message, reason, budgetMs) {
    super(message || code || `request failed with status ${status}`);
    this.status = status;
    this.code = code;
    this.reason = reason;
    // Only set by the 'edit_timeout' stream event, so describeEditError can
    // name the budget the run actually blew through.
...
export async function postEdit({ path, instruction, history, onEvent } = {}) {
  ...
  const handleEvents = (events) => {
    for (const event of events) {
      ...
      if (event.type === 'error') {
        terminalError = new ApiError(undefined, event.code, event.message, event.reason, event.budgetMs);
      }
    }
  };
