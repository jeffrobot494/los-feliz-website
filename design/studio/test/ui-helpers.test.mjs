...
test('describeEditError explains an edit_timeout and names the budget it blew through', () => {
  const message = describeEditError({ code: 'edit_timeout', budgetMs: 180000 });
  assert.match(message, /3m 0s/);
  assert.match(message, /cancelled/);
  assert.match(message, /left unchanged/);
  assert.match(message, /smaller, more specific/);
});

test('describeEditError handles an edit_timeout with no budget attached', () => {
  const message = describeEditError({ code: 'edit_timeout' });
  assert.match(message, /cancelled/);
  assert.doesNotMatch(message, /undefined/);
});

test('describeEditError gives a truncated-rewrite rejection its own explanation', () => {
  const message = describeEditError({ code: 'validation_failed', reason: 'shrunk' });
  assert.match(message, /less than half its original size/);
  assert.match(message, /left unchanged/);
  // Not the generic "(reason)" wording used for every other validation failure.
  assert.doesNotMatch(message, /\(shrunk\)/);
});

test('describeEditError still uses the generic wording for other validation failures', () => {
  const message = describeEditError({ code: 'validation_failed', reason: 'not_html' });
  assert.match(message, /\(not_html\)/);
});
